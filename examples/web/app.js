...
function collapseConsecutive(frames) {
  ...
  for (const f of frames) {
    const key = `${f.mermaid} ${f.svgStyle ?? ""}`;
...
async function generate() {
  ...
  const highlight = $("highlight") ? $("highlight").checked : true;
  ...
  try {
    ...
    for (let i = 0; i < frames.length; i++) {
      ...
      try {
        ...
        svg = result.svg;
        if (frames[i].svgStyle) {
          svg = svg.replace(/<svg([^>]*)>/, `<svg$1><style>${frames[i].svgStyle}</style>`);
        }
      } catch (err) {
